...
⚠ **Expected damage is WOUNDS; casualties are the clipped form.** Damage does
not spill between models, so a Damage-3 weapon against Wounds-1 models removes
one model per hit and not three. Every shipped config is `damage: 1` against
`max_wounds: 1`, where the two columns are the same number.
