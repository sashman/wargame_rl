...
def main() -> None:
    ...
    # ⚠ Without this the teacher DECLARES charges it cannot execute: a factored
    # policy almost never puts the one legal joint rung in every squadmate's
    # top-K, which is the whole reason `apply_charge_decode` exists. Distilling
    # such a teacher on a melee config clones the declaration and not the move.
    charge_decode = bool(int(sys.argv[9])) if len(sys.argv) > 9 else False
    ...
    cache = Path("checkpoints/clone_data") / (
        ...
        f"{'-charge' if charge_decode else ''}"
    ...
    if cache.exists():
        ...
        print(
            ...
            f"(decode_topk={decode_topk}, reallocate={reallocate}, "
            f"charge_decode={charge_decode}) on {config_path}"
        )
