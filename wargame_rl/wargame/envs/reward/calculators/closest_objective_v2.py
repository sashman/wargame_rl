...
class ClosestObjectiveV2Calculator(PerModelRewardCalculator):
    ...
    def __init__(
        ...
        # The per-step memo is keyed on the CONTEXT OBJECT, held strongly and
        # compared by identity. It used to be keyed on
        # `(current_turn, id(distance_cache))` with no reference held: the
        # per-model facade builds and frees a context per decision, CPython
        # recycles the allocation, and the memo served stale counts on 7 of 10
        # consecutive steps. The phase facade builds a fresh context per step
