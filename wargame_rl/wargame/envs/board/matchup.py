...
damage scalar. The moment a matchup number pretends to know range-to-target it
is doing `board/threat.py`'s job badly, so reach appears only as its own
columns: `reach_margin` and `free_rounds`, plus an exchange ratio quoted at two
distances instead of blended into one.
...
def _casualty_scale(damage: int, max_wounds: int) -> float:
    """Fraction of expected wounds that actually removes models.

    `expected_damage` returns **wounds and does not clip them** to what a model
    has left, and its own docstring says so: at `damage > 1` against one-wound
    models it overstates what a volley removes. Damage does not spill between
    models, so a Damage-3 hit on a Wounds-1 model removes one model, not three.

    Exact rather than approximate, because expected damage is *linear* in the
    weapon's Damage -- dividing by `damage` and multiplying by the wounds
    actually usable is the same as having computed it with the clipped value.

    ⚠ Every shipped config is `damage: 1 / max_wounds: 1`, where this is
    identically 1.0. It is written and tested now precisely because nothing
    exercises it: the first heavy weapon added would otherwise inherit a
    threefold overstatement with no failing test to catch it.
    """
    if damage <= 0 or max_wounds <= 0:
        return 0.0
    return float(min(damage, max_wounds)) / float(damage)
