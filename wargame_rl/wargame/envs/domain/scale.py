...
The rules are written in inches. The environment plays on a continuous plane whose unit
is arbitrary, so every rules distance has to be converted before it can be compared
against a coordinate. ``Scale`` is that conversion, and it is the only place the
relationship is defined.

The convention across the codebase:

* **Coordinates, and anything that generates one** -- board dimensions, model and
  objective positions, terrain footprints, deployment zones -- are in **units**.
* **Rules distances** -- move allowance, weapon range, engagement range, base radius,
  objective radius -- are authored in **inches** and converted here.

At the default of one inch per unit the two coincide and conversion is the identity.
"""
...
class Scale:
    ...
    def board_size_in_inches(
        self, board_width: float, board_height: float
    ) -> tuple[float, float]:
        """The board's extent in inches, for reporting alongside unit dimensions."""
        return (self.to_inches(board_width), self.to_inches(board_height))
