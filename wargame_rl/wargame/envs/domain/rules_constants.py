...
The rules are specified in inches; the environment plays in its own coordinate unit.
This module is the runtime source for every universal rules quantity, and
...
a model's Move, a weapon's Range -- vary from model to model and belong in the config,
not here; the defaults for those live alongside their config fields.

Nothing in this module reads a file. The YAML is documentation and is not shipped as a
runtime dependency.
"""
...
# How far a model can see a hidden model.
DETECTION_RANGE_IN = 15.0
