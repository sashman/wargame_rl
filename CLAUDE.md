...
> further, up to 13.7"**. The zone split is still exactly balanced (75/75/75
...
>
> **This pool has a ~8 vp resolution floor.** Per-table `vp_margin` sd is 26–35,
> so even n=45 gives SE 3.9–5.3, and more episodes per table cannot help: the
> variance is *across tables* and only 45 tables exist. **Pairing does not rescue
> a map change**, because the table IS the treatment — `table_05` old and new are
> different boards, so pairing cancels only the part that did not change. That is
> the opposite of pairing two training arms, where identical initial weights make
...
- **The bar on every golden config, re-measured 2026-08-19 (post corpse fix), n=100 at seeds 700000+, as `vp_margin`:** `25v25_maps_two_mode` **−7.1**, `25v25_shooting_opponent` **+13.3**, `25v25_cover_control` **+15.9**, `25v25_maps_coherency` **+105.7**, `25v25_single_phase` / `25v25_curriculum` **+70.3** (identical, as they share a scenario — a useful consistency check). The bar is `squad_march_shoot` on all six. Floors (`random`): −216.3, −124.9, −133.5, −14.7, −256.6 / −256.0.
