...
- **Config inventory for this scenario:** `25v25_shooting_opponent.yaml` (the configuration that beats the bar) and `25v25_cover_control.yaml` (the control it was developed against). Every experiment arm around them was deleted once its question was answered — batch 1/2's, batch 3's `cover_reason`, and batch 4's eight `25v25_beat_*` arms. `git log -- examples/env_config/` restores any of them; `git checkout batch-1-2-configs -- examples/env_config/` for the oldest. Both surviving configs regenerate terrain every episode and set `track_exposure`, which adds `eval/exposure_rate`, `eval/terrain_proximity`, `eval/firepower_ratio` and `eval/fraction_alive`. `exposure_rate` averages over *alive* models, so casualties lower it on their own — read [docs/metrics.md](docs/metrics.md) § Cover metrics before comparing it across configs
