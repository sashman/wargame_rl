...
- ⚠ **THE JOIN FAILS ON BOTH HALVES, THE MEMBERS' HALF IS THE LARGER,
  AND A FROZEN PLANNER IS NOT A FIXED PLAN.** The head and the members
  trained together on A3's shape (CM4) read 0.00 / 0.37 / 0.32 at the
  cap while the same heads' plans, walked by the bar's members, read
  0.72 / 0.68 / 0.72: the head plans under learning members, late and
  not fully, and its own members finish 0.35–0.72 behind its plan
  (leave 0.68–0.77 against the bar's 0.01 on the same plan). The members
  trained under PL1's finished head with its weights frozen (FH1) read
  0.37 / 0.61 / 0.63 — behind A3 with NO commitments (0.70 / 0.80 /
  0.77) and behind the environment's greedy arrived-keeps writer on the
  same reward (`a3_cm`, 0.82 / 0.85 / 0.93) — and the same members with
  their own untrained head committing at random read 0.40 / 0.48 / 0.72:
  on A3's shape the covering walk is reconstructible from the board, so
  the executor learns the geometry and the plan barely enters (the A3
  finding one rung up). Freezing the head's weights did not fix its
  plan: an adaptive head re-commits on what its members do, and PL1's
  weights cover 0.59–0.71 of the objectives on the last turn with the
  learning executor against 0.93–0.97 with the bar's members. **Only the
  WRITER fixes a plan.** The ordering now on file for the same trainer
  and reward — no plan 0.70–0.80, a stable greedy plan 0.82–0.93, an
  adaptive learned plan frozen 0.37–0.63, both learned 0.00–0.37, a
  learned plan with the bar's members 1.000 — says the members want a
  plan that holds still. Do not run the half-step under the joint head;
  do not extend FH1; the next arm holds the head's commitment still (the
  head decides only on an empty slot), and the half-step is climbed by
  the staged route (a plan-only head there, then the members under it),
  where the executor-alone row is the legibility readout A3's shape
  cannot give. Read a split row beside the executor-alone row on every
  frozen-planner arm, and the planner's distinct-on-the-last-turn under
  the executor beside its ceiling under the bar.
