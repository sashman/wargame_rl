...
- **Cover experiment configs:** `25v25_stochastic_terrain_shooting.yaml` (arm A), `_curriculum.yaml` (arm B), `_control.yaml` (arm C). They regenerate terrain every episode and set `track_exposure`, which adds `eval/exposure_rate`, `eval/terrain_proximity` and `eval/fraction_alive`. `exposure_rate` averages over *alive* models, so casualties lower it on their own — read [docs/metrics.md](docs/metrics.md) § Cover metrics before comparing it across configs
