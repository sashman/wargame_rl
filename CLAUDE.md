...
- **Batch 3 re-asks the cover question with all three blockers removed** — `25v25_cover_{control,signal,reason,full}.yaml`, a 2x2 over (`observe_threat_count` × the `models_lost` reward). Terrain is 29 pieces of 3-7 (19.8% hidden vs 5.8%), the policy gets a per-model LOS input, and losses are priced to match kills. **Read `eval/firepower_advantage`, not `eval/exposure_rate`** — exposure counts only our side of the exchange so it falls both when a policy manoeuvres and when it hides. Batch-3 numbers are not comparable to batch 1 or 2; the bar on the new terrain is `squad_march_shoot` at **0.45**, down from 0.63
