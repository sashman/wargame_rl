...
- **The 1.00 bar is an artefact of an opponent that never fires.** The original 25v25 configs use `scripted_advance_to_objective`, which does not shoot — against `scripted_advance_and_shoot` on the same fixed terrain, `squad_march_shoot` falls to 0.60 and `squad_march` 0.80 → 0.24. The `25v25_stochastic_terrain_*` configs use the shooting opponent. Switching a config's opponent invalidates every baseline and agent score measured on it — re-measure both. See [docs/opponent-policies.md](docs/opponent-policies.md)
- **Cover experiment configs:** `25v25_stochastic_terrain_*` (batch 1) and `25v25_terrain_range*` / `25v25_noterrain_range12` (batch 2). They regenerate terrain every episode and set `track_exposure`, which adds `eval/exposure_rate`, `eval/terrain_proximity` and `eval/fraction_alive`. `exposure_rate` averages over *alive* models, so casualties lower it on their own — read [docs/metrics.md](docs/metrics.md) § Cover metrics before comparing it across configs. Batch 2 sets `objective_min_separation`, which changes the scenario distribution, so **the two batches are not comparable**
...
- **Batch 3 answered the cover question and the answer is still no** — `25v25_cover_{control,signal,reason,full}.yaml`, a 2x2 over (`observe_threat_count` × the `models_lost` reward), two seeds each. With 19.8% of the board hidden, a per-model LOS input and priced losses, exposure stayed at 0.092–0.110 across every arm. **Don't re-run this experiment**; see [the report](reports/2026-08-06-cover-signal-reason-geometry.md). What it did find: `models_lost` is worth **+7 vp_margin** with non-overlapping seeds, `observe_threat_count` is **null**, and the penalty made the agent lose *more* models — the opposite of the mechanism it was added for. Batch-3 numbers are not comparable to batch 1 or 2; the bar on the new terrain is `squad_march_shoot` at **0.45**, down from 0.63
- **`eval/firepower_advantage` is broken as specified — do not read it.** As a raw count difference it is dominated by how much engagement happens at all, so `random` (0% win) scores 1.78 while every trained arm scores 1.03–1.29. It needs to be a ratio or normalised by engagement before any conclusion can rest on it
