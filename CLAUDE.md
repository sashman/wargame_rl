...
| **A5-points, the join on the half-step (CM8 / CM8w)** CM4's recipe with `a3_head_rf_pc`'s reward on `a5_points_head_rf_pc.yaml` (six squads of three, five objectives, ten rounds), from scratch (CM8) and with the network warm-started from CM7 s2's PASS checkpoint (CM8w) | start (from scratch v the warm start from A3's join) | **CM8 NULL; CM8w AHEAD on 3/3, PASS on none — the two-stage recipe transfers across army size; the planner's churn in the last third is what caps it** | CM8 **0.03 / 0.06 / 0.16** at 122,880 (per-model scratch without the layer 0.03 / 0.06 / 0.33; s3 behind by 2.8 SE), the head planning the half-step first time from a third on (0.99–1.00) and the soldiers not reading it (blank plan 0.00 / 0.06 / 0.13 = trained; re-commits before arrival 0.63–1.00); CM8w **0.73 / 0.63 / 0.87** at the cap after **0.61 / 0.96 / 1.00** at 81,920 (the best per-model reads on the half-step on file; A4x warm start 0.46 / 0.26 / 0.35; control 0.91 / 0.81 / 0.95), the soldiers following at every read (blank plan 0.00 ×3, arrival at a non-nearest committed objective 0.79–0.93, held 4.5–4.8 of 5) and the planner churning in the last third (re-commits before arrival 0.28 / 0.32 → 0.76 / 0.60 and first-plan 0.81 / 0.84 → 0.42 / 0.62 on the two seeds that fell); 8.6–9.4 turns against the bar's 6.71; the six trainers died on a full disk at 14:20 and were resumed in place | (CM8 at matched rounds; CM8w's three seeds share one warm start; CM8's once-only extension stopped at 210k with its in-run success flat, unread) | [prereg, amendments 15 and 17](reports/2026-09-23-curriculum-commitment-plan-preregistration.md#amendment-17) · [#418](https://github.com/sashman/wargame_rl/issues/418) · [#419](https://github.com/sashman/wargame_rl/issues/419) |
...
- **A3'S JOIN IS CLOSED: PLANNER AND MEMBERS TRAINED TOGETHER PASS THE
  ...
  thing to read.
- **THE TWO-STAGE RECIPE TRANSFERS ACROSS ARMY SIZE, AND THE PLANNER'S
  CHURN IS WHAT CAPS IT.** A3's join (CM7 s2) warm-started onto the
  half-step (six squads, five objectives) reads **0.61 / 0.96 / 1.00** at
  two thirds and 0.73 / 0.63 / 0.87 at the cap, against 0.03 / 0.06 / 0.16
  from scratch and 0.46 / 0.26 / 0.35 for the per-model warm start from
  A4x; the soldiers follow the plan at every read (a blank plan scores
  0.00 on every seed) and hold 4.5–4.8 of 5. From scratch the head plans
  the half-step first time by a third of the budget and the soldiers never
  read it (the ablation is flat) — the seed lottery with no winner in
  three. Between two thirds and the cap the soldiers did not change and
  the planner did (re-commits before arrival 0.28 / 0.32 → 0.76 / 0.60,
  first-plan coverage 0.81 / 0.84 → 0.42 / 0.62 on the seeds that fell).
  Rules: nothing on the half-step trains the joint policy from scratch
  again — warm-start each size step from the rung below's PASS; read a
  joint arm's planner churn (re-commits before arrival, first-plan
  coverage) beside its score at EVERY read and keep the intermediate
  checkpoints, because a joint arm can peak and decay under a churning
  planner (the cap is the verdict, the peak a readout); the next arm is
  planner stability — a KEEP bias or a churn cost on the planning stream,
  one change, pre-registered.
