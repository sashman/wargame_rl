...
| `curriculum/c3b.yaml` | #340 rung **C3**, second arm (#372): `c3.yaml` with `terminate_on_success: false` — success judged on the final board after twelve rounds, because C3's per-model arm found a dash onto the blockers' point that instant termination rewarded. Bar `scripted_escort` **0.990** / alive 0.963 / held 3.97, both facades identical; plain `squad_march_take` **0.280** with half its bodies dead, 2026-09-17. |
