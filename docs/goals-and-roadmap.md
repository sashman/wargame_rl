...
- [~] **Melee combat** — Charge and fight phases exist behind `melee.enabled` (default **False**, an exact no-op). A charge is declared in the command phase by the unit's leader and binds the unit; the referee is all-or-nothing. ⚠ **No agent has ever been trained on it**, and the open work is *how to teach it*, not whether it pays — stumbling into a legal charge by chance happens on **0.27%** of attempts against a scripted policy's 66%. See [melee-teaching-goal.md](melee-teaching-goal.md) for the goal and its success criteria, [melee-outstanding.md](melee-outstanding.md) for every rule still missing and what blocks what, and [melee.md](melee.md) for the design.
