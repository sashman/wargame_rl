...
| `objective_stay` | **per-model** (an ACTION term on the per-model facade: paid to the mover on its own step) | `crowding_exponent` (float, default 1.0) | **Keyed to the commitment layer when it is on** (`StepContext.committed_objective`: only ending inside the unit's committed objective pays). Pays a model for ENDING its step inside an objective, the objective's pot divided by `occupants ** crowding_exponent` (a body alone earns the whole value, three squadmates a third each), enemies ignored. Built for the per-model curriculum's five-objective half-step (2026-09-20), where every trained per-model policy stood still on 0–2% of its decisions on an objective and walked out on 60–81% because the reward paid the same for staying as for leaving: `objective_hold` prices the same standing but is a STATE term on that facade, paid at the close as the army mean. On the phase facade it behaves as a flat-weight `objective_hold` with the pot split, paid every step. |
