...
The phases, attack sequence, terrain rules, and objective mechanics form the core loop that the RL environment aims to model. By default, the environment skips non-movement phases (`skip_phases` config) so each `env.step()` covers only the movement phase. Set `skip_phases: []` to step through all five phases per player turn (command, movement, shooting, charge, fight).

Implemented today: movement, the shooting phase (target selection with range/LOS/engagement masking and the full hit → wound → save → damage sequence, see [shooting.md](shooting.md)), Ruins-style LOS-blocking terrain ([terrain.md](terrain.md)), and objective control scoring VP ([missions-and-vp.md](missions-and-vp.md)). The command, charge, and fight phases allow only "stay". Simplifications worth knowing: models are individuals with no unit coherency, OC is uniform, there is no cover bonus, and a model fires only its first weapon. The Advance move is not implemented — `advanced_this_turn` exists and is read by the shooting mask, but nothing ever sets it. The [roadmap](goals-and-roadmap.md) outlines the path toward charging/melee, morale, and eventually stratagems.
