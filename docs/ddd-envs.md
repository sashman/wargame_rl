...
must drive both directly — and `scripts/behaviour_clone_per_model.py` and `scripts/fit_per_model_critic.py`,
which record a chooser's decisions on the facade (`tests/test_per_model_layering.py`). Hand-stepping it in `debug.py` is #325.
