...
| **Fall Back** | An engaged unit that moves forfeits its shooting and its charge for the turn, and since 2026-08-29 the move is **refereed** (`ActionHandler._enforce_fall_back`): the unit must end unengaged and coherent or the whole move reverts — a reverted fall back never happened, so the unit keeps its shooting. ⚠ The earlier claim that "the geometry was always right" is retracted: `back_off_to_unengaged` cleared only moved endpoints, so a stationary member stayed engaged while its squadmates marched, tearing units to 16.7–20.0" chain gaps on the opponent seat (re-measured 0.5% / worst 6.4" post-referee, the non-melee control's level). Every melee scripted-bar and agent figure measured before the referee is void on melee configs. |
