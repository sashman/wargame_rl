...
| `eval/mean_turns` | `train_per_model.py` | phase-clock turns | Mean turns-to-success over the eval episodes, an episode that never succeeds counted at the clock — the curriculum's speed readout on the in-run curve, the same quantity `just measure-rung` pairs against the script at n=100. Logged only when the eval carries a retimer (always, from the driver). |
...
| `train/ratio_p01` · `train/ratio_p99` | The importance ratio's tails over the update | Inside `1 ± eps_clip` (0.8–1.2) on the first minibatch by construction; a `p99` beyond ~1.5 by the last minibatch is a trust region that no longer binds — pair with `clip_fraction`. ⚠ **At 128 rounds per update the normal is 1.6–1.9** (clip fraction 0.16–0.24): measured on six curriculum runs, it read 1.65–1.83 on the five that passed and 1.68 on the one that failed, so it does not track failure there and is called a fault only when it moves *with* one (`CLAUDE.md` § The per-model curriculum) |
...
the KL anchor (`--kl-ref-coef`, #332): the mean per-decision
...
| `eval/elo` | The same eval games as `vp_margin`, read as a rating against **this config's own opponent, pinned at zero**. ⚠ **A monotone transform of `eval/vp_margin` — it carries no new information about that pairing.** What it adds is a bounded, saturating scale: per-episode margin sd is 45–50, so one blowout moves a mean margin in a way it cannot move a mean win probability. ⚠ **Not a Bradley-Terry rating** — one opponent, no `h_seat`, no `h_turn`, no bootstrap. Comparable across runs only where the opponent and the margin scale match, and **never** to a number from `just elo-table` |
| `self_play/learner_elo` | **Off unless `--self-play`.** A *ladder*: the learner's rating against the pool it has actually played, anchor pinned at zero. Unlike `eval/elo` this is not a transform of anything else — the opponent set moves with the learner, so it can rise while `eval/vp_margin` against a fixed script is flat, and that is the whole point of running a pool. Banked from finished **rollout** episodes, so it costs no extra games |
| `self_play/pool_size`, `self_play/mean_opponent_epoch` | Pool occupancy, and the mean training epoch of the opponents **drawn this epoch** — one draw per rollout env, with the scripted anchor counting as epoch 0. **A pool collapsed onto its newest member is a mechanism failure a score cannot distinguish from a null**, so read these before reading any self-play result. ⚠ **A rising mean is NOT the collapse test** — a healthy pool's mean rises too, because its newest member does. What discriminates is the mean **relative to the newest snapshot**: a pool spanning its run sits far below it (measured: 156 drawn-mean against a newest of 275), while a mean converging on the newest is the collapse. The `self-play pool: N entries spanning epochs A-B` log line is the sharper read, and it names the pool's anchors — a floor is set from the command line and nothing else downstream reveals it. ⚠ And it is an **8-sample point value logged every epoch** — sampling SE is ~33 epochs on a full pool, so read the trend, never one reading |
...
### The plan-only readout — `just measure-plan`

`just measure-plan <head_config> <n> <seed_base> <ckpt...> [members]` scores a
checkpoint on a `head` config twice on identical seeds -- as trained, and
PLAN-ONLY, where the network draws only the commitment decisions and the
scripted `members` policy (default `squad_march_committed`) takes every other
decision, re-planning before each act -- beside the scripted bar. The plan-only
