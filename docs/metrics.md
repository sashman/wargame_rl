...
| `self_play/pool_size`, `self_play/mean_opponent_epoch` | Pool occupancy and the mean epoch of the opponents drawn. **A pool collapsed onto its newest member is a mechanism failure a score cannot distinguish from a null** — read these before reading any self-play result |
