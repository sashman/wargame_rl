...
| `train/grad_clipped_fraction` | Fraction of minibatches where that norm exceeded `max_grad_norm` | Near 1.0 means clipping binds on essentially every update, so `max_grad_norm` — not `lr` — is setting the step size, and raising `lr` alone will change little. Measured at **1.0 with norms 6.5–21.6 against a 0.5 threshold** in early training (2026-08-09, 6 epochs from scratch — not yet read late in a run) |
