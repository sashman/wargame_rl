...
| [Cannot move through enemy models](03-moving.md#making-a-move) | implemented | `domain/movement.py:resolve_move` treats enemy bases as **blockers**: the move stops at base contact. Friendly bases are `passable` — crossable but not endable-on — which the rules do not say, but a squad moving as a body would gridlock on its own front rank otherwise. Verified empirically: a model at (20,20) asked to move +6" at an enemy centred 3" away ends 1.74" along, which is exactly `3 - 0.63 - 0.63`. Gated on `base_radius > 0`, and it defaults to the rules' 32mm infantry base, so this is on in every config. ⚠ This row read *absent — movement ignores occupancy entirely* until 2026-08-22; it predated bases and was never revisited. |
