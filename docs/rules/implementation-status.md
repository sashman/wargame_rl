...
| [Coherency (2" / 9")](03-moving.md#coherency) | **partial** | The predicate is exact — `domain/coherency.py` evaluates both conditions *and* the single-connected-group clause, base to base, over alive models only, at the `coherency` config block's distances (defaulting to the rules' 2"/9"). `just measure-coherency` reports it for any policy. What is enforced is **deployment only**, via `coherency.enforce_at_deployment` (§ Setting up), and it is **off by default** — turning it on is a scenario change that moves every baseline. Nothing checks coherency at the end of a move and nothing destroys models for breaking it, so the two live consequences (§ Making a move's revert, § Regaining coherency's attrition) are still absent. `group_max_distance` + `group_cohesion` remain as the old one-distance approximation, and price only the nearest neighbour. |
