...
| [Advance move](09-movement-phase.md#advance-move) | **partial** | `n_advance_speed_bins` (default **0**, opt-in per scenario) registers an `advance` action slice: `n_movement_angles x n_advance_speed_bins`, valid in the movement phase. Distance is a fraction of the model's Move **plus one D6 rolled per UNIT** at the command→movement boundary — `M + roll` is a MAXIMUM in the rules, not a fixed distance, so the bins span the whole range and the top bin is exactly `M + roll`. ⚠ This admits **dominated** actions (a 4" advance costs the turn's shooting and buys nothing a 4" normal move would not); pruning them was the first design and was wrong, because a unit that cannot stop short cannot advance toward an objective and halt to keep coherency. Advancing sets `advanced_this_turn`, which both shooting masks already read, so it forfeits the turn's shooting — correct here because no weapon has the ability that would permit firing after an advance. ⚠ The slice is appended **after** shooting so no existing action index moves; widening `n_speed_bins` instead would renumber everything (`decode_action` is angle-major, speed-minor) and warm starts load with `strict=False`. ⚠ The dice come from a **dedicated RNG stream**, drawn only when bins > 0, so no existing config's terrain, deployment or combat rolls shift. The roll and the spent-shooting flag are **both in the observation** when bins > 0 (`advance_roll`, normalised by the die so it is not 6x the scale of its neighbours, and `advanced_this_turn` for the value head — the action mask already forbids the shot). ⚠ **An advance scenario CANNOT be paired against a non-advance control.** Turning bins on changes the ACTION space (101 → 149 on `4v4_two_phases`), so the output head is a different shape and the first sampled action can differ; a zero-initialised conditioning path fixes an added *input* but cannot fix an added *action*. Compare unpaired, with more seeds, against a control retrained at the same epoch budget. **Still divergent:** the move type is chosen **per model**, not per unit, so a unit can split — some models advancing while others shoot, which the rules forbid. And an advancing unit should also be unable to declare a charge; the charge phase is a stub, so nothing enforces that yet — whatever implements it must read `advanced_this_turn`. |
