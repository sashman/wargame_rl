...
class TestScriptedAdvanceToObjectivePolicy:
    ...
    def test_models_step_into_objective_from_just_outside(self) -> None:
        """Model just outside objective boundary steps in (no stuck on edge)."""
        cfg = WargameEnvConfig(
            board_width=40,
            board_height=40,
            number_of_wargame_models=1,
        ...
        initial_dist = np.linalg.norm(env.opponent_models[0].location - obj_loc)
        assert initial_dist > radius

        for _ in range(10):
            env.step(WargameEnvAction(actions=[0]))
        final_dist = np.linalg.norm(env.opponent_models[0].location - obj_loc)
        assert final_dist <= radius + 1e-6, "model did not step into objective"
...
class TestActionHandlerExtensions:
    ...
    def test_best_action_toward_small_max_step_length_uses_min_speed(self) -> None:
        """When no speed bin fits within max_step_length, returns min-speed action."""
        cfg = _make_no_opponent_config()
        handler = ActionHandler(cfg)
