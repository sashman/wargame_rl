...
**Units & cover** — `test_units_and_cover` (an unset `unit_id` makes each model its own unit and is *not* `group_id`; a squadmate screen does not block while an enemy one does; a single same-size model gives cover rather than concealment, because the corridor is as wide as the pair; cover costs one point of Ranged Skill and an unmodified 6 still hits; and visibility is symmetric. Every case needs `base_radius > 0` — at radius 0 nothing occludes, which is exactly how a version that never applied the unit rule on the shooting path kept the whole suite green)
