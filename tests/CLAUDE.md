...
**Rules & combat** — `test_shooting_action` · `test_shooting_resolution` · `test_shooting_opponent` · `test_shooting_baseline` · `test_action_masking` · `test_los` · `test_wounds` · `test_killing_reward` · `test_terrain` · `test_terrain_observation` · `test_terrain_render` · `test_random_terrain` · `test_exposure_metric` (exposure, terrain proximity, `firepower_advantage`) · `test_threat_observation` (the `observe_threat_count` column — including that it is measured *after* the move, and that it does not shift `_alive_feature_index`)
