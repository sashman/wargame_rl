...
class TestTheWoundClip:
    ...
    def test_damage_three_against_wounds_two_removes_one_model_per_two_wounds(
        self,
    ) -> None:
        """`min(damage, max_wounds) / damage` -- the partial case between the two above."""
        matchup = matchup_table(
            (_profile(n_models=1, damage=3),), (_profile(n_models=20, max_wounds=2),)
        )[0][0]

        assert matchup.casualties_per_round == pytest.approx(
            matchup.wounds_per_round * 2.0 / 3.0, rel=1e-6
        )
